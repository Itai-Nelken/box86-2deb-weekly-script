...
function package-box86() {
	...
	systemctl restart systemd-binfmt">postinstall-pak || error "Failed to create postinstall-pak! (line 57)"

	#get the just compiled box86 version using the get-box86-version function.
	get-box86-version ver  || error "Failed to get box86 version! (line 60)"
	get-box86-version commit || error "Failed to get box86 commit (sha1)! (line 61)"
	...
	sudo checkinstall -y -D --pkgversion="$BOX86COMMIT" --provides="box86" --conflicts="qemu-user-static" --pkgname="box86" --install="no" make install || error "Failed to run checkinstall! (line 64)"
}

function clean-up() {
	#current date in YY/MM/DD format
	NOWDAY="`printf '%(%Y-%m-%d)T\n' -1`" || error 'Failed to get current date! (line 69)'
	#make a folder with the name of the current date (YY/MM/DD format)
	mkdir -p $DEBDIR/$NOWDAY || error "Failed to create folder for deb! (line 71)"
	...
	mv box86*.deb $DEBDIR/$NOWDAY || sudo mv box86*.deb $DEBDIR/$NOWDAY || error "Failed to move deb! (line 75)"
	#remove the home directory from the deb
	cd $DEBDIR/$NOWDAY || error "Failed to change directory to $DEBDIR/$NOWDAY! (line 77)"
	FILE="`basename *.deb`" || error "Failed to get deb filename! (line 78)"
	FILEDIR="`echo $FILE | cut -c1-22`" || error "Failed to generate name for directory for the deb! (line 79)"
	dpkg-deb -R $FILE $FILEDIR || error "Failed to extract the deb! (line 80)"
	rm -r $FILEDIR/home
	rm -f $FILE || error "Failed to remove old deb! (line 82)"
	dpkg-deb -b $FILEDIR $FILE || error "Failed to repack the deb! (line 83)"
	rm -r $FILEDIR || error "Failed to remove temporary deb directory! (line 84)"
	cd $DEBDIR || error "Failed to change directory to $DEBDIR! (line 85)"
	#compress the folder with the dabe and sha1.txt into a tar.xz archive
	tar -cJf $NOWDAY.tar.xz $NOWDAY/ || error "Failed to compress today's build into a tar.xz archive! (line 87)"
	#remove the box86 folder
	cd $DIR || error "Failed to change directory to $DIR! (line 89)"
	sudo rm -rf box86 || error "Failed to remove box86 folder! (line 90)"
}
...
function upload-deb() {
	...
	cp $DEBDIR/$NOWDAY/box86*.deb $HOME/Documents/weekly-box86-debs/debs/$FILE || error "Failed to copy new deb! (line 97)"
	cp $DEBDIR/$NOWDAY.tar.xz $HOME/Documents/weekly-box86-debs/debs/$NOWDAY.tar.xz || error "Failed to copy new tar.xz archive! (line 98)"
	#remove apt files
	rm $HOME/Documents/weekly-box86-debs/debs/Packages || error "Failed to remove old 'Packages' file! (line 100)"
	rm $HOME/Documents/weekly-box86-debs/debs/Packages.gz || error "Failed to remove old 'Packages.gz' archive! (line 101)"
	rm $HOME/Documents/weekly-box86-debs/debs/Release || error "Failed to remove old 'Release' file! (line 102)"
	#create new apt files
	cd $HOME/Documents/weekly-box86-debs/debs/ || error "Failed to change directory! (line 104)"
	dpkg-scanpackages . /dev/null > Packages || error "Failed to create new 'Packages' file! (line 105)"
	gzip -9c Packages > Packages.gz || error "Failed to create new 'Packages.gz' file! (line 106)"
	#touch Release
	...
	cp $HOME/Documents/box86-2deb-weekly-script/Release-template $HOME/Documents/weekly-box86-debs/debs/Release || error "Failed to copy Release file! (line 114)"
	touch Release
	...
	cd .. || error "Failed to move one directory up! (line 123)"
	git fetch || error "Failed to run 'git fetch'! (line 124)"
	git pull || error "Failed to run 'git pull'! (line 125)"
	git stage debs/ || error "Failed to stage 'debs/'! (line 126)"
	echo "updated deb to $BOX86COMMIT" > commit.txt || error "Failed to create file with commit message! (line 127)"
	git commit --file=commit.txt || error "Failed to commit new deb! (line 128)"
	git push || error "Failed to run 'git push'! (line 129)"
	rm -f commit.txt || error "Failed to remove commit message file! (line 130)"
	cd $DIR || error "Failed to change directory to $DIR! (line 131)"
}
...
while true; do
	...
	if [[ "$NOW" == "Thu" ]]; then
		...
		echo "compile time!"
		compile-box86 || error "Failed to run compile-box86 function! (line 147)"
		package-box86 || error "Failed to run package-box86 function! (line 148)"
		clean-up || error "Failed to run clean-up function! (line 149)"
		...
		upload-deb || error "Failed to upload deb! (line 160)"
